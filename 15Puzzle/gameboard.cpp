...
bool isAdjacent(const GameBoard::Position f, const GameBoard::Position s){
    ...
    if(f.first == s.first){
        ...
        int disance = calcDistance(f.first, s.first);
        if(disance == 1){
            result = true;
        }
    }
...
QVariant GameBoard::data(const QModelIndex &index, int role) const
{
    ...
    return QVariant::fromValue(m_rawBoard[rowIndex].value).toString();
}
...
QString GameBoard::hiddenElementQString() const
{
    return QString::number(m_boardSize);
}
